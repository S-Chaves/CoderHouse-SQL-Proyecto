...
FROM servicios_peliculas sp
	JOIN servicios s ON (sp.id_servicio = s.id_servicio)
	JOIN peliculas p ON (sp.id_pelicula = p.id_pelicula)
WHERE p.puntaje > 5
ORDER BY p.puntaje DESC;

-- Permite ver la cantidad de actores y la suma de sus premios de las películas 
-- que tienen directores que dirigieron más de 15 películas.
